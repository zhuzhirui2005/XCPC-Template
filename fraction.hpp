struct fraction{
    ...
    inline explicit fraction(const string &s):q(1){size_t pos=s.find('.');if(pos==string::npos)p=stoll(s);else{if(pos+1<s.size()){for(int i=0;i<s.size()-1-pos;i++)q*=10;p=(pos?stoll(s.substr(0,pos))*q:0)+stoll(s.substr(pos+1));}else p=stoll(s.substr(0,pos));simplify();}}
